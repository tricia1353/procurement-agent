...
| API 设置 | 配置百度 AI Studio Token、Mouser API Key |
...
2. 前端组件禁止直接请求第三方 API。
3. 后端作为代理层，统一访问 Mouser、百度 AI Studio。
...
Python OpenAI SDK 兼容调用示例（用于后端联调或单独验证 AI Studio 接入）：
...
access_token = os.getenv("LLM_ACCESS_TOKEN")
if not access_token:
    raise RuntimeError("请先设置环境变量 LLM_ACCESS_TOKEN")

client = OpenAI(
    api_key=access_token,
    # OpenAI SDK 会在 base_url 后自动拼接 /chat/completions
    base_url=os.getenv(
        "LLM_OPENAI_BASE_URL",
...
chat_completion = client.chat.completions.create(
    model=os.getenv("LLM_MODEL", "ernie-4.5-turbo-128k"),
    messages=[
        {
            "role": "user",
            "content": "在这里输入你的问题",
...
```bash
export LLM_ACCESS_TOKEN="你的 AI Studio Access Token"
export LLM_MODEL="ernie-4.5-turbo-128k"
# 可选；仅使用 OpenAI SDK 兼容模式时需要，后端 httpx 调用仍使用 LLM_API_URL
export LLM_OPENAI_BASE_URL="https://aistudio.baidu.com/llm/lmapi/v3"
```

安全要求：

1. 禁止把真实 Access Token 写死在代码、PRD、截图或提交记录中。
2. 示例代码必须通过 `os.getenv("LLM_ACCESS_TOKEN")` 或项目配置层读取 Token。
3. `LLM_OPENAI_BASE_URL` 与 `LLM_API_URL` 含义不同：前者用于 OpenAI SDK 兼容模式，默认到 `/llm/lmapi/v3`；后者用于后端 `httpx` 直接请求，默认是 `https://aistudio.baidu.com`。
4. 本项目后端接口默认仍建议使用非流式 `stream: false`，以便统一返回 `{ "message": "..." }`；只有在专门实现流式转发接口时才使用 `stream=True`。
...
```json
{
  ...
  "SearchResults": {
    ...
    "Parts": [
      {
        "MouserPartNumber": "595-NTA53321G0FTTZ",
        "ManufacturerPartNumber": "NTA53321G0FTTZ",
...
```json
{
  "mouser_part_number": "595-NTA53321G0FTTZ",
  "manufacturer_part_number": "NTA53321G0FTTZ",
  "manufacturer": "STMicroelectronics",
  "description": "ARM Microcontrollers",
  "category": "Microcontrollers",
  "image_path": "https://...",
...
| 把 `ProductCompliance` 原样塞进 `product_compliance` | 必须把 `ComplianceName`/`ComplianceValue` 映射为 `name`/`value` |
| 把 `AlternatePackagings` 原样塞进 `alternate_packagings` | 必须把 `APMfrPN` 映射为 `mfr_part_number` |

#### 3.2.1 Mouser 字段映射必做项

Mouser 返回字段经常与前端/Pydantic 模型字段不一致。后端必须在 `services/mouser.py` 或等价解析层完成字段归一化，不能把 Mouser 原始数组直接传给 `Product` 模型，否则会出现 500 或 Pydantic 校验错误。

典型错误表现：

```text
Request failed with status code 500
7 validation errors for Product
product_compliance.0.name Field required
product_compliance.0.value Field required
alternate_packagings.0.mfr_part_number Field required
```
  ...
  "ProductCompliance": [
    {
      ...
      "ComplianceValue": "8542319092"
    }
  ],
  "AlternatePackagings": [
    {
      "APMfrPN": "W25Q256JVEIQ"
    }
  ]
}
```
...
| Mouser 原始字段 | 统一模型字段 | 处理规则 |
|---|---|---|
| `ProductCompliance[].ComplianceName` | `product_compliance[].name` | 字段重命名，缺失时用空字符串 |
| `ProductCompliance[].ComplianceValue` | `product_compliance[].value` | 字段重命名，缺失时用空字符串 |
| `AlternatePackagings[].APMfrPN` | `alternate_packagings[].mfr_part_number` | 字段重命名，缺失时用空字符串 |

复刻要求：

1. `ProductCompliance` 和 `AlternatePackagings` 必须逐项 map，不能直接赋值。
2. Pydantic 模型字段如 `name`、`value`、`mfr_part_number` 如果是必填字段，解析层必须兜底为空字符串，避免真实 API 返回字段不完整导致 500。
3. 对外响应只能返回统一模型字段名，前端不得读取 `ComplianceName`、`ComplianceValue`、`APMfrPN` 这类 Mouser 原始字段。
4. 如果新增 Mouser 字段，也必须先进入统一模型，再由前端消费，避免前后端字段漂移。
...
| 字段 | 输入框下方提示 |
|---|---|
| 百度 AI Studio Access Token | 获取方式：访问 [百度 AI Studio](https://aistudio.baidu.com/account/accessToken) → Access Token → 复制 |
| Mouser API Key | 申请成功后会发送邮件到注册邮箱，可在邮箱里查询 |
...
function getApiKeys() {
  try {
    ...
    return raw
      ? JSON.parse(raw)
      : { llmToken: '', mouserApiKey: '' }
  } catch {
    return { llmToken: '', mouserApiKey: '' }
  }
}
```
...
```json
{
  "questions_by_supplier": {
    "深圳华强电子": [
      {
        "category": "产品与质量",
        "question": "针对 NTA53321G0FTTZ，贵司能否提供原厂授权、质量认证、批次追溯和样品测试资料？",
        "priority": "high",
...
1. 百度 AI Studio Access Token
   - 输入框下方提示：获取方式：访问 [百度 AI Studio](https://aistudio.baidu.com/account/accessToken) → Access Token → 复制
   - 「百度 AI Studio」必须以可点击超链接展示，不得仅展示为普通文本
   - 链接地址：https://aistudio.baidu.com/account/accessToken
   - 点击后在新浏览器标签页打开
   - 链接需使用明显的链接样式，例如蓝色文字、下划线或 hover 状态
2. Mouser API Key
   - 输入框下方提示：申请成功后会发送邮件到注册邮箱，可在邮箱里查询
...
```text
输入物料型号，如: NTA53321G0FTTZ, ESP32-WROOM-32, LM7805
```

快捷型号：

```text
NTA53321G0FTTZ、ESP32-WROOM-32、LM7805、ATmega328P
```
...
| 字段 | 数据 | 显示规则 |
|---|---|---|
| 产品图片 | `image_path` | 有值则显示图片，否则不渲染 |
| 制造商料号 | `manufacturer_part_number` | 文本显示 |
| 制造商 | `manufacturer` | 文本显示 |
| 描述 | `description` | 文本显示 |
| 第一档价格 | `price_breaks[0].price` | 直接输出原始字符串，**不加 `$` 前缀**；如需展示货币，读取 `price_breaks[0].currency` 拼在后面，如 `18.50 USD` |
| 库存 | `availability_in_stock` | 数字显示 |
| 卷带标签 | `reeling` | 布尔显示 |

价格显示踩坑点：

> 前端不能对 `price_breaks[0].price` 硬编码 `$` 前缀。Mouser 返回值为原始字符串（如 `"18.50"`），货币单位由 `currency` 字段表示（如 `"USD"`）。部分地区或供应商使用欧元等其他货币，硬编码 `$` 会导致货币标识错误。
...
### 17.9 Mouser 调用失败
Mouser API 返回的某些字段（如 LifecycleStatus）值为 null，part.get("LifecycleStatus", "") 在 key 存在但值为 None 时不会用默认值，直接把 None 传给了 Pydantic 的 str 类型字段，导致校验失败。

修复方式：改用 part.get(key) or "" 确保 None 被替换为空字符串。
